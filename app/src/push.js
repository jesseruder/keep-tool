...
// True when this phone is registered with this server, which is the persisted flag
// the background sweep is switched off by. Deliberately not aged out: the daemon
// keeps a device until Expo says it is gone, so a phone that has not launched for a
// week is still receiving pushes.
function isPushActive(saved, server) {
  const record = savedRecord(saved);
...
async function syncRegistration({
  ...
  if (granted !== 'granted') {
    ...
    if (remove && isPushActive(saved, server)) {
  ...
  // reaches, and notifying everything twice.
  const fallback = (reason) => (isPushActive(saved, server)
    ? state('registered', { tokenTail: tokenTail(saved.token), registeredAt: saved.registeredAt, fresh: true, stale: reason })
    : state('unavailable', { reason }));

  let token = '';
  try { token = await getToken(); }
  catch (error) { return fallback(text(error && error.message, 120) || 'no push token'); }
...
async function unregisterDevice({ config, storage, remove } = {}) {
  ...
  if (token && (!config || isPushActive(saved, config.server))) await clearRegistration(storage);
