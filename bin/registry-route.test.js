...
test('the registry route exists only where the daemon listens for nodes', async (t) => {
  ...
  // Every other route still refuses a node: the node API is these, the hook route and deploy-self.
  const forNodes = on.filter((entry) => (entry.allow || []).includes('node')).map((entry) => entry.path);
  assert.deepEqual(forNodes, ['/api/registry', '/api/hook', '/api/registry/ping', '/api/deploy-self']);
