...
test('the reviewer toolbar leaves its live Restart to restartControls and explains a missing pane', async () => {
  ...
  // restartControls renders the same forced Restart it renders for every session.
  assert.equal(reviewerRestartHTML(ctx(live)), '');
  assert.equal(reviewerRestartHTML(ctx(live, [{ sessionId: 'r1', status: 'done' }])), '');
