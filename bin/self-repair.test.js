...
test('retired, disabled, on-demand and self-repair rows never produce a signature', () => {
  ...
  const snapshot = snapshotOf([
    ...
    { name: 'loop-stalls', consecutiveFailures: 40, lastError: 'event loop stalled over 5s', lastErrorAt: NOW },
    { name: 'node-hook-queue', consecutiveFailures: 40, lastError: 'aws1: 200 hook events queued (at cap)', lastErrorAt: NOW },
  ...
  assert.deepEqual([...selfRepair.EXCLUDED].sort(), ['git-pull', 'inflight', 'lint', 'loop-stalls', 'node-hook-queue', 'runs']);
});
