...
const rows = [];
const fields = ['client', 'event', 'mode', 'session', 'pane', 'selected', 'target', 'related', 'visibility', 'reason', 'targetKey'];
function record(events, now = Date.now()) {
  ...
  for (const event of events) {
    ...
    for (const field of ['at', 'inputRecentMs', 'pointerId', 'x', 'y']) if (Number.isFinite(event[field])) row[field] = event[field];
