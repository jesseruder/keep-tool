...
test('running panel leaves out a paneless session that waits only on its card, unless it runs outside Keep or was marked running', () => {
  const source = fs.readFileSync(path.join(__dirname, '../web/app/app.js'), 'utf8');
  ...
  assert.deepEqual(Array.from(ctx.runningItems(), (item) => item.id).sort(), ['background', 'live', 'marked', 'outside', 'working'],
    'only a paneless session waiting on its card alone is left out');
});
