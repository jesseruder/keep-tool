// Tool definitions, mirroring docs/claude-in-chrome-tools.txt name-for-name and
// property-for-property so a model's existing habits carry over unchanged.
//
// Data only: server.js registers these through the SDK's low-level request handlers so
// the JSON schemas reach the client exactly as written here. test/tools.test.js parses
// the contract file and checks this list against it.
//
// Dropped (Anthropic cloud features with no local meaning): shortcuts_list,
// shortcuts_execute, switch_browser, list_connected_browsers, select_browser.
// Added: browser_status.

const MAX_RESULT_META = { "anthropic/maxResultSizeChars": 60000 };
...
const FILE_UPLOAD_SIZE_LIMIT = "The combined size of all files in a single call must stay under 10 MB.";

export const TOOLS = [
  {
    name: "file_upload",
    // Everything is on one machine, so the bridge reads the paths itself; `files` stays
    // in the schema for fidelity and is ignored.
    description: `${FILE_UPLOAD_INTRO} Pass \`paths\` of absolute paths on this machine; the bridge reads them directly, so a missing file, a non-regular file, a file with more than one hard link, or a total over 10 MB is rejected. ${FILE_UPLOAD_SIZE_LIMIT}`,
    inputSchema: {
      type: "object",
      properties: {
        paths: {
        ...
        files: {
          type: "array",
          items: UPLOAD_FILE_SCHEMA,
          description:
            "Populated by the client from `paths` after it reads them under its own file-read permissions; do not set this yourself.",
        },
      },
      required: ["ref", "tabId"],
    },
...
* Make sure to click any buttons, links, icons, etc with the cursor tip in the center of the element. Don't click boxes on their edges unless asked.`,
    inputSchema: {
      ...
      properties: {
        ...
        duration: {
          ...
          description: `The number of seconds to wait. Required for \`wait\`. Maximum ${MAX_WAIT_SECONDS} seconds.`,
        },
        scroll_direction: {
          type: "string",
        ...
        scale: {
          ...
          minimum: MIN_SCALE,
          maximum: MAX_SCALE,
          description: `For \`screenshot\` and \`zoom\` only. Scale factor in [${MIN_SCALE}, ${MAX_SCALE}] for the returned image; 1 (default) returns the viewport at CSS-pixel size, 0.5 returns an image at half the width and height (~quarter of the tokens). Coordinates are ALWAYS in the full-resolution coordinate frame (reported with every scaled screenshot), never in the scaled image's own pixels.`,
        },
        repeat: {
          type: "number",
          minimum: 1,
  ...
  },
  {
    name: "browser_batch",
    description:
      "Execute a sequence of browser tool calls in ONE round trip. Each item is {name, input} where input is exactly what you'd pass to that tool standalone. Actions execute SEQUENTIALLY (not in parallel) and stop on the first error. Use this tool extensively to quickly execute work whenever you can predict two or more steps ahead — e.g. navigate, click a field, type, press Return, screenshot. Screenshots and other images are returned interleaved with outputs; coordinates you write in THIS batch refer to the screenshot taken BEFORE this call. Tools that act on a page require an explicit tabId inside a batch. browser_batch cannot be nested.",
    inputSchema: {
      type: "object",
      properties: {
        actions: {
  ...
  },
  {
    name: "browser_status",
    description:
      "Report the state of the Browser Bridge itself: whether the native host is running, the extension's version, this session's tab group and its tabs, and every other agent session sharing this browser. Use it when a browser tool fails and you need to know whether the browser side is up. It works even when the host is down — it then says so instead of failing.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
];
...
/** Tools that act on a page and therefore need an explicit tabId inside a batch. */
