...
test("the plan writes the launcher and the manifests and registers both CLIs", (t) => {
  ...
  // Each registration is a remove (ignored when nothing was there) and then an add, so a
  // moved checkout replaces the old path instead of keeping it.
  ...
  assert.deepEqual(claude[1].args.slice(0, 5), ["mcp", "add", "--scope", "user", "browser"]);
  assert.equal(claude[1].optional, false);
  assert.equal(claude[1].args[5], "--");
  assert.equal(claude[1].args[6], process.execPath);
  assert.match(claude[1].args[7], /mcp\/server\.js$/);
  assert.equal(claude[2].env.CLAUDE_CONFIG_DIR, path.join(env.HOME, ".claude-secondary"));
  assert.deepEqual(claude[2].args.slice(0, 2), ["mcp", "remove"]);
  assert.equal(claude[3].env.CLAUDE_CONFIG_DIR, path.join(env.HOME, ".claude-secondary"));
  assert.deepEqual(claude[3].args.slice(0, 2), ["mcp", "add"]);
  ...
  assert.deepEqual(codex[0].args, ["mcp", "remove", "browser"]);
  assert.equal(codex[0].optional, true);
  assert.deepEqual(codex[1].args.slice(0, 3), ["mcp", "add", "browser"]);
  assert.equal(codex[1].args[3], "--");
});

test("uninstall removes both browsers' manifests and unregisters the servers", (t) => {
  const env = fakeHome(t);
  ...
  for (const command of plan.commands) {
    assert.ok(command.args.includes("remove"), command.label);
    assert.notEqual(command.optional, true, "an uninstall failure is worth reporting");
  }
});
...
test("--dry-run prints every file and command and changes nothing", (t) => {
  ...
  assert.match(output, /run {2}.*mcp remove --scope user browser {3}\(failure ignored\)/);
  assert.match(output, /run {2}.*mcp add --scope user browser --/);
  assert.ok(
    output.indexOf("mcp remove --scope user browser") < output.indexOf("mcp add --scope user browser"),
    "the remove has to come first",
  );
