...
- **Stop.** A node session is never stopped gracefully: the graceful stop proves the session idle from its job ledger against the transcript on the machine that writes it, which the daemon cannot read on another node. It is stopped one of two ways, both of which capture and signal the pane's process tree on the node through its host (`process` and `signal`), with the stop proven from the node's process table, including the after-the-fact proof on a retry.
  ...
  - **The rate-limit queue's parked stop** (`parkedForce`), for a session parked on its limit. It is accepted only with `expectedSourceAccountId` and `expectedRateLimitAt`; the only exception is a Retry of a transaction already past its stop, which has nothing left to stop. It is refused for a pane on the daemon's own node, whose sessions keep the graceful stop. It is nothing else of `ownerForce`: every preflight above and every artifact proof runs as for an unforced transfer. Inside the injection lock, immediately before the first signal, Keep proves the session still parked from fresh reads of the node. It reads the node's own transcript tail, never the mirror. That tail must carry the same rate-limit event, no user turn after it, and no running tool, background work, question or plan, and the daemon's row for the session must pass the same restart checks a terminal-limit session on this machine passes. Keep then reads the pane's screen, which must show no running turn (`esc to interrupt`), no dialog and no draft; a ghost suggestion is told from a draft the way a message send tells it. Finally it reads the tail again, which must not have changed since the first read. A running turn, background work, a transcript that changed and a node that did not answer are transient, so the entry waits and tries again. A limit that cleared or was replaced, a turn by the person since the limit, a draft and a dialog park it (a limit the queue's own snapshot already shows cleared cancels the entry before the transfer is asked, as it does for any entry).
