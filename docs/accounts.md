...
Such an entry usually names no rate-limit event, so nothing cancels it for a limit that cleared. What stands in for that is the person: if the session is used again before the retry lands, the entry is cancelled with "session was used since it was queued" instead of stopping a session somebody went back to work in. Otherwise it runs until it lands, until it gives up at `KEEP_HANDOFF_QUEUE_MAX_MIN`, or until you cancel it.
