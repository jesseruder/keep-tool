...
Before each attempt the queue rechecks that the entry still describes the session. A session that has left the state, that is already on the target, or that a person has since moved to some third account is retired rather than transferred from wherever it is now. If the rate limit has cleared — the window reset, or the person simply went back to work — the entry is cancelled with "rate limit cleared" instead of stopping a live session and typing a continuation into it; the one exception is a handoff transaction that has already started, which is always allowed to finish. **Cancel** takes effect immediately, even while that session's own transfer is in flight: the attempt's result is only written if the entry has not changed underneath it.
