...
When an account has more than one session parked on its weekly limit, the account controls offer **Move N rate-limited sessions to ‹account›**, one button per same-provider destination, with that destination's usage next to it. For this machine's own sessions the button does not transfer anything. It enqueues each of them in the daemon's transfer queue (`~/keep/.keep/handoff-queue/`, one file per session). A session on another node cannot wait in that queue, because the queue never force-stops a session and a node's session moves only when forced. The batch returns those with their pane, and the console moves them one at a time with the same Owner-forced transfer that session's own **Continue on another account** runs, showing `Moving 2 of 7…` on the button. When it finishes, one toast says how many moved, how many were queued, and which failed or were skipped, with the reason for each.
