...
a `checks-fallback` is only a reserve when it sits outside the pool, since a pool
member is spent whenever the pool is), and the rate-limit handoff below — asks
...
- The preferred account (`automationAccounts[purpose]`) wins only when a current
  reading shows at least the budget governor's minimum headroom (`KEEP_REVIEW_MIN_HEADROOM`,
  default 10%, or the model family's `minHeadroom`) on its `week`, the model's own bucket
  and `5h`. Otherwise it is ranked like the others: the lowest `week` percent wins
  (ties: the model's own weekly bucket, then the `5h` window). An account is spent when
  its `week` or `5h` is at 100%, or when the model has its own bucket (Fable:
  `Fable wk`) and that one is. Opus, Sonnet and Haiku have no bucket of their own, and
  work that names no model is capped by the `week` and `5h` only.
- Readings come from `~/keep/.keep/usage-cache.json`. A window at 100% whose reset is
  unknown or still ahead is spent whatever the reading's age. Otherwise a missing or
  stale (over 30 minutes) reading is *unknown*: ranked after every account with a
  current reading that shows room.
...
**A transfer you start yourself is forced.** Every click on **Continue on another account**, its **Retry**, a **Reopen** onto another account, and `keep handoff --force` send `ownerForce`. Keep then asks for no proof that the session is idle and types nothing into it: it captures the pane's process tree by pid and start time, journals it on the handoff record, sends SIGTERM and then SIGKILL to exactly those processes, copies the conversation once all of them are gone, and resumes it on the target. A retry also stops any process an earlier forced attempt captured that is still running, and after a daemon crash the stop is only accepted once every journalled process is gone. You can see whether the session is working; a turn in progress is interrupted. What still refuses is what would make the move fail or corrupt it: the target not logged in or set up incompatibly, a permission class or model that cannot be reproduced, overlapping or aliased profiles, a rollout that changes while it is copied, or a process that is not the one inspected. For Codex, a forced move reads which child threads belong to the conversation from each rollout's own `session_meta` instead of the job ledgers. A forced request never joins the retry queue: its refusal comes back to the click. The rate-limit queue and every other automatic transfer never send `ownerForce`. On this machine they keep the full verification below; for a session on another node the queue has one narrower stop of its own, the parked stop described under [A session on another node](#a-session-on-another-node).
...
When an account has more than one session parked on its weekly limit, the account controls offer **Move N rate-limited sessions to ‹account›**, one button per same-provider destination, with that destination's usage next to it. For this machine's own sessions the button does not transfer anything. It enqueues each of them in the daemon's transfer queue (`~/keep/.keep/handoff-queue/`, one file per session). A session on another node that is not already in the queue is not added to it: the button is Owner asking for the move now, and the queue would wait for the parked stop's proofs. The batch returns those with their pane and the limit it saw, and reports one the [automatic policy](#automatic-transfer-on-the-weekly-limit) has already queued as *already queued* instead. The console then moves the returned ones one at a time with the same Owner-forced transfer that session's own **Continue on another account** runs, showing `Moving 2 of 7…` on the button. Each of these transfers also sends the source account and the limit the batch saw, so a session that resumed or moved in the meantime is refused instead of being stopped mid-turn. When it finishes, one toast says how many moved, were queued or are still running, and which need recovery, failed or were skipped, with the reason for each.

A queue entry is only patience. Every thirty seconds the daemon takes the entries that are due and asks for the **same** transfer the single-session button asks for, through the same `handoffSession`, with every preflight intact. The queue never bypasses a check, never launches or types anything, and passes `force` only when the request that queued the session carried it. The one stop it asks for by itself is `parkedForce`, and only with `expectedSourceAccountId` and `expectedRateLimitAt` (below); an entry that names no source account or no limit never sends it. Which stop an attempt asks for follows the pane the session is in at that attempt, not the one it was queued from: a session that came back to this machine during a backoff gets the graceful stop, and one that went to a node gets the parked stop.
...
Each queue tick enqueues rate-limited Claude sessions, without `force`, and the ordinary queue rules above take it from there. That includes a session whose pane is on another node: the policy reads its row from the daemon's mirror of its transcript, and queues it for the parked stop (`parkedForce`, [A session on another node](#a-session-on-another-node)) with the source account and the limit event it saw. A node session whose mirror names no limit event is not queued. Everything else in this section applies to it unchanged. A `rateLimitHandoff` key is the owner's opt-in for a source account: **every** rate-limited session on it moves to the named target while that target has room for the session's model, and to the [automation pool](#automation-pool)'s best account when it does not. On a source with no key, only **unattended** sessions — ones Keep started by itself, whose pane carries `unattended` or `reviewer` — move to the pool's best account; the owner's own sessions stay put. A target is never the session's own source. The model comes from the pane's launch record, else the transcript. A session the queue moved less than 30 minutes ago (`MOVE_COOLDOWN_MS`) is not moved again automatically, so two accounts whose spent 5h windows the snapshot has not caught cannot bounce it back and forth. The pair must name two existing accounts for the same provider and must not name the same account twice; an unusable key is reported on stderr once per tick and ignored rather than guessed at. A target whose `week`, `5h`, or the session model's own weekly bucket the usage snapshot shows at 100% (with that window's reset still ahead) is not moved onto, however old the reading; a missing reading is never read as exhausted. When nothing has room the session is held and the tick logs `policy held N session(s)`. A parked or cancelled entry is waiting on a person, so the policy never re-queues it — only the console's **Retry** does.

With no `rateLimitHandoff` key and an empty automation pool (a single Claude account, or `"automationPool": []`), nothing is ever queued automatically, on this machine or on a node, and the only way a session moves is the single-session button, the batch button, or `keep handoff`. With a pool, the policy runs with no key at all, for unattended sessions only.
...
### A session on another node

A Claude or Codex session whose pane is on another node (a pane id `<id>@<node>`) is transferred on that node, to another account's copy there; the session stays on its node. Each node holds its own copy of an account and is logged in separately, so every question about the machine the session runs on is asked of that node, never answered from the daemon's copies:

- **Preflight.** The pane comes from the node's own answer to a fresh pane listing, the agent's identity (pid, start time, whether it owns the pane) from the node's process table read at that moment, and the session row from the node's transcript (the Claude tail, or the Codex rollout's `session_meta` and tail). The Claude model is the launch model unless the tail's newest genuine assistant record names another base model; the node's tail is all that is read, so a `/model` switch the tail does not show is not seen.
- **Target login on the node.** The node runs `claude auth status --json` (Codex: `codex login status`) through its login shell under the target account's profile and answers only whether it is logged in and which config directory it reports. A target logged in on the daemon but not on the node is refused as *not logged in on ‹node›*, with the source left running.
- **Setup, trust, Codex policy.** The node prepares the source's shared setup, compares the two accounts' setups, reads the Codex resume policy from the source's root rollout (and later the target's copy), checks the Codex working directory, and carries the folder-trust answer into the target's `.claude.json`, each on its own copies.
- **Stop.** A node session is never stopped gracefully: the graceful stop proves the session idle from its job ledger against the transcript on the machine that writes it, which the daemon cannot read on another node. It is stopped one of two ways, both of which capture and signal the pane's process tree on the node through its host (`process` and `signal`), with the stop proven from the node's process table, including the after-the-fact proof on a retry.
  - **Owner's forced stop** (the console's button, **Retry**, a **Reopen** onto another account, `keep handoff --force`), which asks for no idle proof.
  - **The rate-limit queue's parked stop** (`parkedForce`), for a session parked on its limit. It is accepted only with `expectedSourceAccountId` and `expectedRateLimitAt`; the only exception is a Retry of a transaction already past its stop, which has nothing left to stop. It is refused for a pane on the daemon's own node, whose sessions keep the graceful stop. It is nothing else of `ownerForce`: every preflight above and every artifact proof runs as for an unforced transfer. Inside the injection lock, before anything is captured, Keep proves the session still parked from fresh reads of the node. It reads the node's own transcript tail, never the mirror. That tail must carry the same rate-limit event, no user turn after it, and no running tool, background work, question or plan, and the daemon's row for the session must pass the same restart checks a terminal-limit session on this machine passes. Keep then notes the pane's input counter and reads its screen, which must show no running turn (`esc to interrupt`), no dialog and no draft; a ghost suggestion is told from a draft the way a message send tells it, with the probe's two keys sent conditional on that counter. It reads the tail again, which must not have changed, and the counter, which must have moved by exactly the probe's own keys. The injection lock keeps Keep out of the pane, not a person at the terminal, so the proof is taken once more at the last moment: after the stop's final read of the node's process table, the pane's counter (still no key but the probe's), its screen (no running turn, no dialog) and the tail (unchanged, same limit) are read again, and only then is the capture journalled and the first SIGTERM sent. A refusal at any of these points leaves nothing journalled and nothing signalled. What remains between that last look and the signal is the journal write and the signal request to the node; a key typed inside that gap is lost, as it would be under Owner's force. A running turn, background work, a transcript that changed and a node that did not answer are transient, so the entry waits and tries again. A limit that cleared or was replaced, a turn by the person since the limit, a key typed into the pane during the proof, a draft and a dialog park it (a limit the queue's own snapshot already shows cleared cancels the entry before the transfer is asked, as it does for any entry).
  - **Anything else** is refused first, before the node or the session is asked anything and before any transfer record is written: an unforced transfer (`keep handoff` without `--force`) as *A session on ‹node› is transferred only when forced*, and a parked stop that names no source account or no limit as *A parked-session transfer on ‹node› must name the source account and the limit it is for*.
- **Copy.** The conversation's files are walked from the source account's directory to the target's on the same node, through the same `artifacts` verb a `keep move` uses with both ends on that node: listed, read and staged piece by piece, published, and compared by digest, with the source listed again so a session that wrote while it was carried is refused. The source's copy is then released, so a transfer back may replace it. Before the stop, a target account that already holds other bytes for the session, which no move or transfer put there, refuses the transfer. There is no job ledger to rebind for a node session.
- **Relaunch and continuation.** The target starts in the same pane through the node's host, with its launch prepared by the node (`prepare-launch`: its launcher, and the target's shared setup and MCP configuration). Its identity is verified from the node's table and its SessionStart record, and the continuation is delivered through the node like any other delivery there, receipted by the node from the target account's transcript.

Two rules hold for every transfer, on a node or not. A Retry of a transfer that committed its `/exit`, signalled a forced stop or got past the stop stops only the source it recorded: a pane found running again (a restart, a reopen, another transfer's launch) is refused with the record left as it is, never stopped. And the target's launch is journalled (`targetLaunchIntent`: the transaction, pane, account and a digest of the command) before the host is asked to relaunch; when the host ran the relaunch but its reply was lost, Retry adopts the pane that carries this transaction's mark, this session and the target account, and verifies it exactly as a fresh launch, instead of launching a second agent.

A node that does not answer — the pane listing, its process table, any of these questions — is a slow host: the transfer refuses as *host request timed out …*, which is transient, and nothing is written. If the node stops answering mid-transfer, the record is left `recovery-needed` at the step it reached; **Retry** continues from there once the node answers again. Abandoning a transfer past its stop reads the node's process table too, and is refused while the node cannot be asked. A node whose host predates these questions is refused by name before anything else is asked: *predates account transfers (its artifacts verb is version N; a transfer needs 3)*, and for a Codex session *predates Codex account transfers (its transcript verb is version N; a Codex transfer needs 4)*: update keep-tool there and reload its host. A Pi session on a node, and a pane on one node for a session whose location record names another, are refused.
