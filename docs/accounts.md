...
A transfer whose typed `/exit` did land, but whose confirmation was lost to a later host timeout, is recovered rather than stranded. The restart journals `sourceExitTypedAt` on the record just before it types the `/exit`, after every stop check has passed. On the next attempt, a `recovery-needed` record still in `stopping-source` with no `sourceStopVerifiedAt` proves the stop after the fact when all of these hold: the `/exit` was typed; the pane is the record's own, still carries this session, was not relaunched by a handoff, and is no longer alive; and a fresh `ps` snapshot has no process with the recorded `sourceAgentPid` and `sourceAgentPidStart` (the pid gone, or reused by a process that started at another time). The record then gets `sourceStopVerifiedAt` with `sourceStopVerifiedBy: "post-hoc-ps"` and the ordinary recovery path runs — copy, relaunch on the target, one continuation. A `ps` that fails or comes back empty keeps it refused as *Source exit could not be verified*, which is transient; a recorded process that is still running, a pane that is not the record's, and a source that exited without Keep ever typing its `/exit` (a refusal followed by a crash skipped the job-ledger checks the stop makes) all stay *Source exit was not verified by the handoff transaction; recovery is blocked*.
