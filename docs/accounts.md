...
- **Stop.** A transfer of a node session is always Owner's forced stop (the console's button, **Retry**, a **Reopen** onto another account, `keep handoff --force`). The forced stop captures and signals the pane's process tree on the node through its host (`process` and `signal`), and the stop is proven from the node's process table, including the after-the-fact proof on a retry. An unforced transfer — the rate-limit queue, `keep handoff` without `--force` — is refused first, as *A session on ‹node› is transferred only when forced*, before the node, the session or an existing transfer record is read or written: the graceful stop proves the session idle from its job ledger against the transcript on the machine that writes it, which the daemon cannot read on another node.
