...
- **`keep reports`** — the open groups (`--area app-server` for yours, `--all` for every
...
- **Slack** — the read-only `mcp__jesse__slack_*` tools: `slack_search` takes
  ...
  in **`#dev-issue-reports`**.

What to do with them:

1. **On an incident**, search both for the alert's window (from an hour before the first
   firing to now) with the symptom's words — `error`, `crash`, `loading`, `slow`, `login`, `can't save`, `publish`, the
   feature the failing query serves — and look at
   `keep reports --area app-server`. A user report tells you the impact is real and often
...
2. **When a `user-reports` event wakes you**, read the group (`keep reports show
   ...
   - `keep reports mark <group> real --card <id> -m "why"` — after filing the card
     ...
     links). In your area — errors, failed saves or loads, login, passes, slowness, missing data,
     anything the API or worker serves — it is yours to investigate, and impact happening
