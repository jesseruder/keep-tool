...
is hidden by colour: the theme's cursor colours are set to the background, and the
console's own theme is put back exactly when the chain ends (a theme change mid-chain
becomes the one put back). The predictor writes nothing to the terminal, so the pane's
own cursor visibility (`CSI ? 25 h/l`) is never touched. A resize, or a prompt row
moved by scrollback trimming, ends the chain. Echoes are acknowledged oldest keystroke
