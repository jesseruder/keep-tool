...
| --- | --- |
| `sched:<name>:<hash8>` | a scheduler has `consecutiveFailures >= minFailures` (5) on one normalized error, its fault still stands (the latest attempt failed, or the last failure is inside health's recent window: the longer of an hour and two cadences, capped at a day), and that signature was first seen at least `minAgeMin` (30) minutes before its latest failure |
...
five real attempts that all failed. A `sched:` signature is a candidate while that
fault stands (`health.faultStands`, the same rule that reads the row as failing rather
than recovered; see Health states in the reference): its latest attempt failed, or its
last failure is inside the recent window, at least an hour. This tick runs every five
minutes and the skips land every minute, so it would almost never see such a row with
a failure as its latest record, and a scheduler whose real work comes hourly and fails
every time must still get a card. Past the window a recovered row opens nothing. Dropping out of the candidates does not clear an open card: that
...
Two rows use it. `discord` records it only when the Castle MCP gateway it reads from
is not there to ask — refused, reset, unresolvable, timed out, or not serving the
`discord_recent` tool yet: the `GatewayUnavailable` its `poll()` swallows into the
status file, or one that reaches the scheduler's catch. A gateway that is there and
failing (401/403, 5xx, a JSON-RPC or tool error) does not qualify, and neither does
anything on this side: a failed headers helper, no credential, a malformed page, a
classifier that refused, a decisions file that would not write are real failures and
go red. `usage` records it when every failure
